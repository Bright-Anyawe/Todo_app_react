...
const SideBar = () => {
  ...
  const handleProjectClick = (projectName) => {
    // Navigate programmatically to the ProjectComponent page for the selected project
    navigate(`/display/project/${projectName.toLowerCase()}`);
  };
...
<div className="projects">
          ...
          {Array.isArray(projects) &&
            projects.slice(startIndex).map((project, index) => (
              <div className="project" key={index + startIndex}>
                <span>#</span>
                <h3>{project.name}</h3>
